...
export default function () {
  ...
  return () => {
    ...
    getAppPath().then((appPath: string) => {
      ...
      fileAction
        .readDir(`${appPath}/electron-resume-platform/src/assets/template`)
        .then(async (files: string[]) => {
          // 3. 构造模版列表
          if (files.length > 0) {
            let templateList: TSTemplate.Item[] = [];
            for (let idx = 0; idx < files.length; idx++) {
              const base64URL = await fileAction.read(`${appPath}/electron-resume-platform/src/assets/template/${files[idx]}`, 'base64');
              templateList.push({
